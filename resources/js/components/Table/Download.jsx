...
function DownloadOption({ type, icon }) {
    const { data, rows, csvConfig, pdfConfig, columns, page, selected } =
        useTable();
    ...
    const download = (
        ...
        if (downloadType === "pdf")
            exportAsPdf({ ...options, config: pdfConfig });
        else if (downloadType === "csv")
            exportAsCsv({ ...options, config: csvConfig });
    };
    ...
    return (
        ...
        >
            ...
            {selected.length > 0 && (
                <DropDown.Option
                    onClick={() => download(type, rows, null, selected)}
                >
